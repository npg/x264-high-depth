...
typedef struct x264_frame
{
    ...
    int     i_lines_lowres;
    pixel_t *plane[3];
    pixel_t *filtered[4]; /* plane[0], H, V, HV */
    pixel_t *lowres[4]; /* half-size copy of input frame: Orig, H, V, HV */
    uint16_t *integral;

    /* for unrestricted mv we allocate more data than needed
     * allocated data are stored in buffer */
    pixel_t *buffer[4];
    pixel_t *buffer_lowres[4];

    x264_weight_t weight[16][3]; /* [ref_index][plane] */
    pixel_t *weighted[16]; /* plane[0] weighted of the reference frames */
    int b_duplicate;
...
typedef void (*x264_deblock_inter_t)( pixel_t *pix, int stride, int alpha, int beta, int8_t *tc0 );
typedef void (*x264_deblock_intra_t)( pixel_t *pix, int stride, int alpha, int beta );
typedef struct
{
