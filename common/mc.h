...
typedef void (* weight_fn_t)( pixel_t *, int, pixel_t *, int, const struct x264_weight_t *, int );
...
typedef struct
{
    void (*mc_luma)(pixel_t *dst, int i_dst, pixel_t **src, int i_src,
                    int mvx, int mvy,
                    int i_width, int i_height, const x264_weight_t *weight );

    /* may round up the dimensions if they're not a power of 2 */
    pixel_t* (*get_ref)(pixel_t *dst, int *i_dst, pixel_t **src, int i_src,
                        int mvx, int mvy,
                        int i_width, int i_height, const x264_weight_t *weight );

    /* mc_chroma may write up to 2 bytes of garbage to the right of dst,
     * so it must be run from left to right. */
    void (*mc_chroma)(pixel_t *dst, int i_dst, pixel_t *src, int i_src,
                      int mvx, int mvy,
                      int i_width, int i_height );

    void (*avg[10])( pixel_t *dst, int, pixel_t *src1, int, pixel_t *src2, int, int i_weight );

    /* only 16x16, 8x8, and 4x4 defined */
    void (*copy[7])( pixel_t *dst, int, pixel_t *src, int, int i_height );
    void (*copy_16x16_unaligned)( pixel_t *dst, int, pixel_t *src, int, int i_height );

    void (*plane_copy)( pixel_t *dst, int i_dst,
                        pixel_t *src, int i_src, int w, int h);

    void (*hpel_filter)( pixel_t *dsth, pixel_t *dstv, pixel_t *dstc, pixel_t *src,
                         int i_stride, int i_width, int i_height, int16_t *buf );

    /* prefetch the next few macroblocks of fenc or fdec */
    void (*prefetch_fenc)( pixel_t *pix_y, int stride_y,
                           pixel_t *pix_uv, int stride_uv, int mb_x );
    /* prefetch the next few macroblocks of a hpel reference frame */
    void (*prefetch_ref)( pixel_t *pix, int stride, int parity );

    void *(*memcpy_aligned)( void *dst, const void *src, size_t n );
    void (*memzero_aligned)( void *dst, int n );

    /* successive elimination prefilter */
    void (*integral_init4h)( uint16_t *sum, pixel_t *pix, int stride );
    void (*integral_init8h)( uint16_t *sum, pixel_t *pix, int stride );
    void (*integral_init4v)( uint16_t *sum8, uint16_t *sum4, int stride );
    void (*integral_init8v)( uint16_t *sum8, int stride );

    void (*frame_init_lowres_core)( pixel_t *src0, pixel_t *dst0, pixel_t *dsth, pixel_t *dstv, pixel_t *dstc,
