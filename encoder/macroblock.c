...
{
    int nz;
    pixel_t *p_src = &h->mb.pic.p_fenc[0][block_idx_xy_fenc[idx]];
    pixel_t *p_dst = &h->mb.pic.p_fdec[0][block_idx_xy_fdec[idx]];
...
void x264_mb_encode_i8x8( x264_t *h, int idx, int i_qp )
{
    ...
    int y = 8 * (idx>>1);
    int nz;
    pixel_t *p_src = &h->mb.pic.p_fenc[0][x+y*FENC_STRIDE];
    pixel_t *p_dst = &h->mb.pic.p_fdec[0][x+y*FDEC_STRIDE];
...
void x264_mb_encode_8x8_chroma( x264_t *h, int b_inter, int i_qp )
{
    ...
    for( int ch = 0; ch < 2; ch++ )
    {
        pixel_t  *p_src = h->mb.pic.p_fenc[1+ch];
        pixel_t  *p_dst = h->mb.pic.p_fdec[1+ch];
...
void x264_predict_lossless_4x4( x264_t *h, pixel_t *p_dst, int idx, int i_mode )
{
    int stride = h->fenc->i_stride[0] << h->mb.b_interlaced;
    pixel_t *p_src = h->mb.pic.p_fenc_plane[0] + block_idx_x[idx]*4 + block_idx_y[idx]*4 * stride;
...
void x264_predict_lossless_8x8( x264_t *h, pixel_t *p_dst, int idx, int i_mode, pixel_t edge[33] )
{
    int stride = h->fenc->i_stride[0] << h->mb.b_interlaced;
    pixel_t *p_src = h->mb.pic.p_fenc_plane[0] + (idx&1)*8 + (idx>>1)*8*stride;
...
void x264_macroblock_encode( x264_t *h )
{
    ...
    else if( h->mb.i_type == I_8x8 )
    {
        ...
        {
            pixel_t  *p_dst = &h->mb.pic.p_fdec[0][8 * (i&1) + 8 * (i>>1) * FDEC_STRIDE];
    ...
    else if( h->mb.i_type == I_4x4 )
    {
        ...
        {
            pixel_t  *p_dst = &h->mb.pic.p_fdec[0][block_idx_xy_fdec[i]];
...
{
    int i_qp = h->mb.i_qp;
    pixel_t *p_fenc = h->mb.pic.p_fenc[0] + (i8&1)*8 + (i8>>1)*8*FENC_STRIDE;
    pixel_t *p_fdec = h->mb.pic.p_fdec[0] + (i8&1)*8 + (i8>>1)*8*FDEC_STRIDE;
    int b_decimate = h->mb.b_dct_decimate;
    int nnz8x8 = 0;
    int nz;
...
{
    int i_qp = h->mb.i_qp;
    pixel_t *p_fenc = &h->mb.pic.p_fenc[0][block_idx_xy_fenc[i4]];
    pixel_t *p_fdec = &h->mb.pic.p_fdec[0][block_idx_xy_fdec[i4]];
    int nz;
