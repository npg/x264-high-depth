...
static NOINLINE pixel_t *x264_weight_cost_init_luma( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, pixel_t *dest )
{
...
static NOINLINE unsigned int x264_weight_cost( x264_t *h, x264_frame_t *fenc, pixel_t *src, x264_weight_t *w )
{
    unsigned int cost = 0;
    int i_stride = fenc->i_stride_lowres;
    int i_lines = fenc->i_lines_lowres;
    int i_width = fenc->i_width_lowres;
    pixel_t *fenc_plane = fenc->lowres[0];
    ALIGNED_ARRAY_8( pixel_t, buf,[8*8] );
    int pixoff = 0;
    int i_mb = 0;
...
void x264_weights_analyse( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead )
{
    ...
    pixel_t *mcbuf = x264_weight_cost_init_luma( h, fenc, ref, h->mb.p_weight_buf[0] );
    ...
    if( weights[0].weightfn && b_lookahead )
    {
        ...
        pixel_t *src = ref->buffer_lowres[0];
        pixel_t *dst = h->mb.p_weight_buf[0];
                                    ...
                                    int dist_scale_factor, int do_search[2], const x264_weight_t *w )
{
    ...
    ALIGNED_ARRAY_8( pixel_t, pix1,[9*FDEC_STRIDE] );
    pixel_t *pix2 = pix1+8;
    x264_me_t m[2];
...
#define TRY_BIDIR( mv0, mv1, penalty ) \
    { \
        ...
        { \
            ...
            pixel_t *src1 = m[0].p_fref[hpel_idx1] + ((mv0)[0]>>2) + ((mv0)[1]>>2) * m[0].i_stride[0]; \
            pixel_t *src2 = m[1].p_fref[hpel_idx2] + ((mv1)[0]>>2) + ((mv1)[1]>>2) * m[1].i_stride[0]; \
...
lowres_intra_mb:
    if( !fenc->b_intra_calculated )
    {
        ...
        pixel_t *pix = &pix1[8+FDEC_STRIDE - 1];
        pixel_t *src = &fenc->lowres[0][i_pel_offset - 1];
        const int intra_penalty = 5;
        int satds[3];

        memcpy( pix-FDEC_STRIDE, src-i_stride, 17 * sizeof(pixel_t) );
        for( int i = 0; i < 8; i++ )
